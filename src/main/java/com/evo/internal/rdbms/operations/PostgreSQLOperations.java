...
public class PostgreSQLOperations implements IOperationsPerRdbms {
	...
	public int getCurrentPort(Connection conn) throws SQLException {
		try (PreparedStatement stmt = conn.prepareStatement("SELECT setting FROM pg_settings WHERE name = 'port';");
	...
	public void dropSchema(Connection conn, String schema) throws SQLException {
		try (PreparedStatement stmt = conn.prepareStatement("DROP SCHEMA IF EXISTS " + schema + " CASCADE")) {
			stmt.execute();
		}
	}
	...
	public void changeToSchema(Connection conn, String schema) throws SQLException {
		try (PreparedStatement stmt = conn.prepareStatement("SET search_path " + schema)) {
			stmt.execute();
		}
	}
	...
	public boolean existEvoMigrationHistoryTable(Connection conn, String schema) throws SQLException {
		return this.existTable(conn, schema, MigrationHistoryTable.TABLE_NAME);
	}
	
	@Override
	public void createEvoMigrationHistoryTable(Connection conn, String schema) throws SQLException {
		try (PreparedStatement stmt = conn.prepareStatement("CREATE TABLE IF NOT EXISTS " + schema + ".__evo_migration " +
				"(" +
					MigrationHistoryTable.COL_MIGRATION_TYPE + " character(1) NOT NULL, " +
					MigrationHistoryTable.COL_VERSION + " character varying(20) NOT NULL, " +
					MigrationHistoryTable.COL_DESCRIPTION + " character varying(255) NOT NULL DEFAULT ''::character varying, " +
					MigrationHistoryTable.COL_FILENAME + " character varying(255) NOT NULL, " +
					MigrationHistoryTable.COL_CHECKSUM + " integer NOT NULL, " +
					MigrationHistoryTable.COL_EXECUTED_AT + " timestamp without time zone NULL DEFAULT NULL, " +
					MigrationHistoryTable.COL_IS_CURRENT + " boolean NOT NULL DEFAULT false, " +
					MigrationHistoryTable.COL_IS_TARGET + " boolean NOT NULL DEFAULT false, " +
					MigrationHistoryTable.COL_LAST_EXECUTED_STMT_POS + " integer NOT NULL DEFAULT 0, " +
					MigrationHistoryTable.COL_LAST_EXECUTED_STMT + " character varying(1000) NOT NULL DEFAULT ''::character varying, " +
					MigrationHistoryTable.COL_LAST_EXECUTED_STMT_LINE + " integer NOT NULL DEFAULT 0, " +
					MigrationHistoryTable.COL_LAST_EXECUTED_STMT_FIRST_CHAR + " integer NOT NULL DEFAULT 0, " +
					MigrationHistoryTable.COL_SUCCESS + " boolean NOT NULL DEFAULT false" +
				");" +
				"CREATE INDEX migration_type_version " +
					"ON " + MigrationHistoryTable.TABLE_NAME + " (" +
